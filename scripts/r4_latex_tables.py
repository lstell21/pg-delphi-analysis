...
def build_panel_table(submissions, groups=None, label="tab:panel_characteristics",
    ...
    if brief:
        ...
        notes.append(r"Background, sector, and career stage are separate questions, "
                     r"so their counts need not agree: two panellists work outside a "
                     r"university while one reports a non-academic background.")
    # The size goes inside the caption argument, matching the body. It cannot go
    # outside: LaTeX's \@caption issues \normalsize before \@makecaption, so a
    # surrounding declaration never reaches the caption. At 12pt this caption ran
    # the float past \textheight.
    lines.append(rf"\caption{{{size}\textbf{{Composition of the expert panel.}} Characteristics of the {total} panellists who completed the final "
...
def build_statements_appendix(consensus, comparison) -> str:
    ...
    lines.append(r"\begin{longtable}{@{}l >{\raggedright\arraybackslash}p{0.43\textwidth} c c c l@{}}")
    ...
    lines.append(r"\toprule")
    header = r"ID & Statement & R3 & R4 & $n$ (R4) & Status \\"
    lines.append(header)
    lines.append(r"\midrule")
...
def build_rankings_table(rankings) -> str:
    ...
    lines.append(r"\begin{longtable}{@{}>{\raggedright\arraybackslash}p{0.52\textwidth} r r r r r@{}}")
    lines.append(r"\caption{\textbf{Ranking of preparedness measures by pandemic phase.} "
                 ...
                 r"mean rank when their $n$ differ. The IQR is the interquartile range of "
                 r"the ranks a measure received, so a small value means panellists placed "
                 r"it consistently and a large one that they did not. $N$ is the number of "
                 r"panellists who ranked a phase at all, and varies across phases because a "
                 r"panellist could rank one phase and leave another blank; $n$ is the "
                 r"number who ranked that particular "
    ...
    lines.append(r"\toprule")
    header = r"Preparedness measure & $n$ & First & Mean rank & IQR & Borda \\"
    lines.append(header)
    lines.append(r"\midrule")
    lines.append(r"\endfirsthead")
    lines.append(r"\caption[]{\emph{(continued)}}\\")
    ...
    for pi, (phase, pdata) in enumerate(phases.items()):
        ...
        lines.append(rf"\multicolumn{{6}}{{@{{}}p{{\linewidth}}@{{}}}}{{\textbf{{{desc}}} "
