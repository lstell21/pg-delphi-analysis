...
1. ``recommendations_main.tex`` -- all 40 consensus recommendations in the
   ...
   and the Round 4 median, IQR, endorsement and rater count. This is the paper's
   main result and sits in the manuscript itself, so it carries the full text
   rather than short labels. Wording comes from ``r4_statements``; the ratings
   are read back out of ``round4_consensus_table.tex``, so this stays in sync
   when the Round 4 pipeline is re-run.

   The % 5--6 column is the top-two share, the percentage of raters choosing 5
   or 6. Median and IQR saturate once agreement is high -- 38 of 40 medians sit
   at the scale maximum -- so they stop discriminating between statements. The
   top-two share still does, running from 77% to 100%, and it lets a reader
   judge the ceiling effect directly. It is read from
   ``data/generated/consensus_stats.json``, which carries the full rating
   distribution. It is deliberately NOT called "endorsement": the manuscript
   uses that word in its ordinary sense, and naming the statistic with it made
   several sentences about the median read as claims about this column.
...
CONSENSUS_STATS_JSON = (PROJECT_ROOT / "data" / "generated"
...
def read_endorsement() -> dict:
    """{id: pct} -- share of raters choosing 5 or 6, from the Round 4 stats."""
    import json
    with open(CONSENSUS_STATS_JSON, encoding="utf-8") as f:
        data = json.load(f)
    out = {}
    for sid, row in data["statements"].items():
        dist = row["distribution"]
        n = row["n_rated"]
        if not n:
            continue
        out[sid] = round(100.0 * (dist["5"] + dist["6"]) / n)
    return out


def build_recommendations_table(statements: dict, stats: dict, endorse: dict) -> str:
    rows = []
    for section in "ABCD":
        ids = sorted(i for i in statements if i.startswith(section))
        if not ids:
            continue
        rows.append(r"\multicolumn{7}{@{}l}{\textbf{Section %s: %s}}\\"
                    % (section, SECTION_TITLES[section]))
        rows.append(r"\addlinespace[2pt]")
        for i in ids:
            median, iqr, n, status = stats.get(i, ("--", "--", "--", "--"))
            pct = endorse.get(i)
            rows.append(r"%s & %s & %s & %s & %s & %s & %s \\"
                        % (i, tex_escape(statements[i]), AUDIENCE.get(i, "--"),
                           median, iqr, "--" if pct is None else "%d" % pct, n))
            rows.append(r"\addlinespace[3pt]")
        rows.append(r"\addlinespace[4pt]")
    body = "\n".join(rows).rstrip()
    # The column carries the name the prose uses, so "the top-two share" in the
    # Discussion points at something a reader can find. It is NOT called
    # "endorsement": the manuscript uses that word in its ordinary sense, and
    # naming the statistic with it turned sentences about the median, such as
    # "the two statements with slightly lower endorsement", into claims about
    # this column, which they are not.
    #
    # The name sits on two lines and the unit on the second, so the cell is only
    # as wide as "share (%)". On one line it overran the margin.
    top2 = r"\begin{tabular}[b]{@{}c@{}}Top-two\\share (\%)\end{tabular}"
    header = (r"ID & Recommendation & Must act & Median & IQR & "
              + top2 + r" & $n$ \\")
    # Seven columns, so the six internal gaps cost 12pt each at the default
    # \tabcolsep. Naming the top-two share column needs about 10pt more than
    # "% 5--6" did; trimming the gaps to 5pt frees 12pt, which keeps the
    # statement wording at its full 0.43\textwidth.
...
codebook (\protect\hyperref[sec:app:coding]{Supplementary Note~18}) assigns from the extracted challenge and
...
def main() -> None:
    ...
    endorse = read_endorsement()
    rec_table = build_recommendations_table(statements, stats, endorse)
