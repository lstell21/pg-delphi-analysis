...
def build_recommendations_table(statements: dict, stats: dict, endorse: dict) -> str:
    ...
    body = "\n".join(rows).rstrip()
    header = r"ID & Recommendation & Must act & Median & IQR & \% 5--6 & $n$ \\"
...
Note~9). Ratings run from 1 (strongly disagree) to 6 (strongly agree), and all 40
met the consensus criterion of IQR~$\leq$~1. \%%~5--6 is the share of raters
choosing either top point, which still separates statements once the median
saturates. $n$ varies because partial responses were retained and panellists
could abstain. Supplementary Tables~4 and~10 give the Round~3 comparison and a
