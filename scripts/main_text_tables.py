...
1. ``recommendations_main.tex`` -- all 40 consensus recommendations in the
   ...
   Endorsement is the share of raters choosing 5 or 6. Median and IQR saturate
   once agreement is high -- 38 of 40 medians sit at the scale maximum -- so
   they stop discriminating between statements. Endorsement still does, running
   from 77% to 100%, and it lets a reader judge the ceiling effect directly. It
   is read from ``data/generated/consensus_stats.json``, which carries
...
def build_recommendations_table(statements: dict, stats: dict, endorse: dict) -> str:
    ...
    body = "\n".join(rows).rstrip()
    # The column is headed with the word the prose uses. It was headed "% 5--6",
    # which left "the endorsement column" in the Discussion pointing at nothing a
    # reader could find.
    header = r"ID & Recommendation & Must act & Median & IQR & Endorsement & $n$ \\"
    # Seven columns, so the six internal gaps cost 12pt each at the default
    # \tabcolsep. Heading the endorsement column with the word the prose uses
    # rather than "% 5--6" needs about 25pt more; tightening the gaps to 4pt
    # frees 24pt of that and 0.42\textwidth for the statement column the rest,
    # which keeps the wording at full width instead of narrowing it.
    return r"""%% Auto-generated by scripts/main_text_tables.py -- do not edit by hand.
\begingroup\footnotesize\setlength{\tabcolsep}{4pt}
\begin{longtable}{@{}l >{\raggedright\arraybackslash}p{0.42\textwidth} l c c c c@{}}
...
40 met the consensus criterion of IQR~$\leq$~1. Endorsement is the share of
raters choosing 5 or 6, which still separates statements once the median
saturates. $n$ varies because partial responses were retained and panellists
